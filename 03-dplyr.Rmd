...
columns. For this we'll use `mutate()` (without the pipe for single operations):
...
Mutate is also useful to change character strings. E.g. spelling `F` and `M` 
in order to make the data (and the diagrams we draw later ) more self-explanatory:

```{r mutate-abbr}
suverys <- mutate(surveys,
  sex = if_else(
    condition = is.na(sex),
    true = "undetermined",
...
same target colum as `if_else`'s source column. The raw CSV file must not be 
changed, but cleaning the data within R's environment and with explicitely coded
steps is fine.
...
3 different cases, we need 2 `if_else` calls.
...
>
> ```{r, answer=TRUE}
> # ?count or the auto-complete pop-up
...
```{r}
surveys_complete <- filter(surveys,
...
To make sure that everyone has this many rows/observations and columns/variables:

```{r complete-dimensions}
